...
function update() {
	...
	if ($("cmp_tudou").checked) {
		vars.push("type=tudou");
	}
	if ($("cmp_qiyi").checked) {
		vars.push("type=qiyi");
	}
	...
	if (musicUrl) {
		$("musicErr").innerHTML = "";
		vars.push(SrcType + encodeURIComponent(musicUrl));
	}else {
	...
	$("yycode").value = "[flash]http://afp.qiyi.com/main/c?db=qiyiafp&bid=1,1,1&cid=1,1,1&sid=0&url=" + url + "&.swf|560|420[/flash]";
